...
def record_scalar_metrics(metrics, step_time_delta, per_device_tflops, lr, cumalative_token_count, token_count):
    ...
    metrics["scalar"].update({"token_count/current": token_count})
    metrics["scalar"].update({"token_count/cumalative": cumalative_token_count})
...
def init_train_state(model, tx, config, key):
    ...
    model_vars = model.init({"params": key, "dropout": key}, jnp.ones(input_shape), jnp.ones(input_shape))
    state = train_state.TrainState.create(apply_fn=model.apply, params=model_vars["params"], tx=tx)
    return state
...
def record_activation_metrics(output_metrics, intermediate_outputs, config):
    ...
    if config.scan_layers:
        ...
        for layer_num in range(config.num_decoder_layers):
            ...
            output_metrics["scalar"][f"activ_mean/layer_{layer_num:03d}"] = metrics_dict["activation_mean"][0][
                layer_num
            ]
            output_metrics["scalar"][f"activ_stdev/layer_{layer_num:03d}"] = metrics_dict["activation_stdev"][0][
                layer_num
            ]
    else:
        for layer_num in range(config.num_decoder_layers):
            layer = intermediate_outputs["intermediates"]["decoder"][f"layers_{layer_num}"]
            output_metrics["scalar"][f"activ_fraction_zero/layer_{layer_num:03d}"] = layer["activation_fraction_zero"][
                0
            ]
...
def predict_step(inputs, state, rngkey, model, config):
    ...
    def tokens_ids_to_logits(flat_ids, flat_cache):
        ...
        flat_logits, new_vars = model.apply(
            {"params": state.params, "cache": flat_cache},
            flat_ids,
            None,
            enable_dropout=False,
            decode=True,
...
def train_loop(config, state=None):
    ...
    tx = optax.adam(
        max_utils.create_learning_rate_schedule(learning_rate=config.learning_rate, warmup_steps=config.warmup_steps)
    )
    ...
    tokenized_prompts = encode_strings([config.prompt], config.max_predict_length, sp_tokenizer)
    ...
    for step in np.arange(get_first_step(state), config.steps):
        ...
        token_count = jnp.count_nonzero(example_batch["inputs"])
        global_token_count += token_count
        with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
            state, metrics, nextrng = p_train_step(model, config, state, example_batch, nextrng)
